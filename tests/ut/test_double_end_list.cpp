...
namespace {
   ...
   SCENARIO("double end queue") {
      ...
      WHEN("enqueue a elem, should return ok") {
         REQUIRE(double_end_list::ok == list.enqueue(new job{8}));
      ...
      WHEN("enqueue many elems, should return ok") {
         REQUIRE(double_end_list::ok == list.enqueue(new job{1}));
         REQUIRE(double_end_list::ok == list.enqueue(new job{2}));
         REQUIRE(double_end_list::ok == list.enqueue(new job{3}));
         AND_THEN("the list is not empty") {
            REQUIRE_FALSE(list.empty());
         }
