...
struct multi_future {
   ...
   inline auto valid() const noexcept -> bool { return valid_; }
   ...
   auto then(F_CALLBACK&& callback, F_FAIL&& on_fail) && noexcept -> future_awaiter {
      ...
      auto awaiter = std::make_shared<multi_future_awaiter<F_CALLBACK, F_FAIL, Xs...>>
         ( *context_
   ...
   auto on_succeed(F_CALLBACK&& callback) && noexcept -> future_awaiter {
      return std::move(*this).then(std::forward<F_CALLBACK>(callback), [](auto){});
   }
