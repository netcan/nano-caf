...
using first_arg_t = typename callable_trait<F>::template arg<0>;
   ...
   template<typename F>
   struct verify_behavior_<F, std::enable_if_t<std::is_base_of_v<__atom_signature, std::decay_t<first_arg_t<F>>> && std::is_same_v<const msg_id_t, decltype(from_msg_type_to_id<std::decay_t<first_arg_t<F>>>::msg_id)>>> {
...
struct behavior {
   ...
   behavior(Args&&...args) {
      ...
      static_assert((verify_behavior<Args>::value && ...));
      //static_assert(((is_msg_or_atom<first_arg_t<Args>>) && ...), "the first argument must be an atom or a message type");
